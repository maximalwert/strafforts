class RacesController < ApplicationController
  def index
    ...
    if params[:distance_or_year].blank?
      ...
      error_message = "Could not find requested distance or year '#{distance}'."
      raise ActionController::BadRequest, error_message if race_distance.nil?
  ...
  def get_counts_by_distance # rubocop:disable AccessorMethodName
    ...
    ApplicationHelper::Helper.all_race_distances.each do |race_distance|
      ...
      result = {
        race_distance: race_distance[:name],
        count: items.nil? ? 0 : items.size,
        is_major: race_distance[:is_major]
      }
      results << result
    end
    render json: results
