...
class AthletesControllerTest < ActionDispatch::IntegrationTest
  test 'should be a bad request when POST publicize_profile with id or username that do not exist' do
    exception = assert_raises(ActionController::BadRequest) do
      post '/athletes/12345678/publicize_profile'
    end
    assert(exception.message.include?('Could not find requested athlete'))
  end

  test 'should set is_public correctly when POST publicize_profile' do
    # Scenario 1.
    ...
    post '/athletes/123/publicize_profile'
    athlete1.reload
    assert_not(athlete1.is_public)

    post '/athletes/123/publicize_profile', params: { is_public: true }
    athlete1.reload
    ...
    post '/athletes/456/publicize_profile', params: { is_public: true }
    athlete2.reload
    assert(athlete2.is_public)

    post '/athletes/456/publicize_profile'
    athlete2.reload
