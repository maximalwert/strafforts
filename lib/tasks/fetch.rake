namespace :fetch do
  desc 'Fetch data for a particular athlete by ID. Usage: rake fetch:athlete MODE=[all/latest] ID=[athlete_id]'
  ...
  desc 'Fetch best efforts for all athletes. Usage: rake fetch:best_efforts MODE=[all/latest]'
  task best_efforts: :environment do
    fetch(ENV['MODE'], %w[best-efforts])
  end

  desc 'Fetch races for all athletes. Usage: rake fetch:races MODE=[all/latest]'
